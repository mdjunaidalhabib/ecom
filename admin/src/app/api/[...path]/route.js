...
// dev এ .env থেকে subdomain, production এ host header থেকে auto
const DEV_SUBDOMAIN = process.env.NEXT_PUBLIC_SUBDOMAIN || "";

function getMissingBackendUrlResponse() {
  return NextResponse.json(
    { success: false, message: "Backend API URL is missing. Please set BACKEND_API_URL." },
    { status: 500 },
  );
}
...
function copyRequestHeaders(req) {
  ...
  // ✅ KEY FIX: backend কে জানাও কোন tenant এর request
  // Production এ nginx আসল host পাঠায় (shop1.glorixbd.com)
  // Dev এ .env থেকে subdomain পাঠাই
  if (DEV_SUBDOMAIN) {
    // development mode
    headers.set("x-tenant-subdomain", DEV_SUBDOMAIN);
  } else if (incomingHost) {
    // production: real host forward করো
